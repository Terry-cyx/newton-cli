# robot_allegro_hand — B-route via newton-cli run-script.
# This example needs per-step Python (custom kernels drive the grasping policy)
...
"=== robot_allegro_hand via newton-cli run-script (B-route) ===" | Tee-Object -FilePath $logFile

& $py -m newton_cli run-script $runner     --forward robot_allegro_hand --forward 20     --artifact-dir $outDir     --timeout 240 --json 2>&1 | Tee-Object -FilePath $logFile -Append
