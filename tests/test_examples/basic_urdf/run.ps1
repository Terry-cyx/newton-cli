# basic_urdf — 4 ANYmal quadrupeds from URDF via newton-cli.
# Requires newton[importers] extra (URDF parser).
...
"=== basic_urdf via newton-cli ===" | Tee-Object -FilePath $logFile

"--- prep: resolve quadruped.urdf path ---" | Tee-Object -FilePath $logFile -Append
...
"`n--- sim run (100 frames @ 100 fps, XPBD, cpu) ---" |
    Tee-Object -FilePath $logFile -Append
...
& $py $viz $stateOut $snapshot $summary --title "basic_urdf (CLI)" 2>&1 |
    Tee-Object -FilePath $logFile -Append
