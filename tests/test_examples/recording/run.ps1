# recording — B-route via newton-cli run-script.
# This example needs per-step Python (USD recording demo (empty test_final))
...
"=== recording via newton-cli run-script (B-route) ===" | Tee-Object -FilePath $logFile

& $py -m newton_cli run-script $runner     --forward recording --forward 30     --artifact-dir $outDir     --timeout 240 --json 2>&1 | Tee-Object -FilePath $logFile -Append
