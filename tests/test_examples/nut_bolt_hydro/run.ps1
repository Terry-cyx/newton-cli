# nut_bolt_hydro — single nut+bolt assembly with hydroelastic SDF contacts.
...
"=== nut_bolt_hydro via newton-cli ===" | Tee-Object -FilePath $logFile
...
& $py $viz $stateOut $snapshot $summary --title "nut_bolt_hydro (CLI)" 2>&1 |
    Tee-Object -FilePath $logFile -Append
