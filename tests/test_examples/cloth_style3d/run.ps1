# cloth_style3d — B-route via newton-cli run-script.
# This example needs per-step Python (SolverStyle3D._precompute(builder) needs the live builder reference)
...
"=== cloth_style3d via newton-cli run-script (B-route) ===" | Tee-Object -FilePath $logFile

& $py -m newton_cli run-script $runner     --forward cloth_style3d --forward 30     --artifact-dir $outDir     --timeout 240 --json 2>&1 | Tee-Object -FilePath $logFile -Append
