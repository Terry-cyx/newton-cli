# cloth_rollers — B-route via newton-cli run-script.
# This example needs per-step Python (custom kernels drive the rollers)
...
"=== cloth_rollers via newton-cli run-script (B-route) ===" | Tee-Object -FilePath $logFile

& $py -m newton_cli run-script $runner     --forward cloth_rollers --forward 300     --artifact-dir $outDir     --timeout 600 --json 2>&1 | Tee-Object -FilePath $logFile -Append
