# cloth_bending — curved cloth surface dropped onto ground (VBD).
# Needs newton[importers] to load curvedSurface.usd at prep time.
...
"=== cloth_bending via newton-cli ===" | Tee-Object -FilePath $logFile
...
& $py -m newton_cli sim run `
    ...
    --solver-arg particle_self_contact_radius=0.2 `
    --solver-arg particle_self_contact_margin=0.35 `
...
& $py $viz $stateOut $snapshot $summary --title "cloth_bending (CLI)" 2>&1 |
    Tee-Object -FilePath $logFile -Append
