# basic_plotting — 4 NV humanoids dropped via MJCF + MuJoCo solver.
# The original example plots per-step energy diagnostics; we only verify
# that the physics runs stably and bodies stay above ground.
...
"=== basic_plotting via newton-cli ===" | Tee-Object -FilePath $logFile
...
"`n--- sim run (60 frames @ 60 fps, 10 substeps, MuJoCo, cuda:0) ---" |
    Tee-Object -FilePath $logFile -Append
...
& $py $viz $stateOut $snapshot $summary --title "basic_plotting (CLI)" 2>&1 |
    Tee-Object -FilePath $logFile -Append
