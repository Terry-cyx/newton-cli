# diffsim_ball — B-route via newton-cli run-script.
# This example needs per-step Python (an autograd loop trains the launch parameters)
...
"=== diffsim_ball via newton-cli run-script (B-route) ===" | Tee-Object -FilePath $logFile

& $py -m newton_cli run-script $runner     --forward diffsim_ball --forward 30     --artifact-dir $outDir     --timeout 240 --json 2>&1 | Tee-Object -FilePath $logFile -Append
