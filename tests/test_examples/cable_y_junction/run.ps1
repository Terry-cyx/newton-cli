# cable_y_junction — Y-shaped rod graph, 3 branches of 20 rods, tip pinned.
...
"=== cable_y_junction via newton-cli ===" | Tee-Object -FilePath $logFile
...
"`n--- sim run (60 frames @ 60 fps, 10 substeps, VBD iter=5, cuda:0) ---" |
    Tee-Object -FilePath $logFile -Append
...
& $py $viz $stateOut $snapshot $summary --title "cable_y_junction (CLI)" 2>&1 |
    Tee-Object -FilePath $logFile -Append
