# mpm_snow_ball — B-route via newton-cli run-script.
# This example needs per-step Python (custom kernels for snow rendering)
...
"=== mpm_snow_ball via newton-cli run-script (B-route) ===" | Tee-Object -FilePath $logFile

& $py -m newton_cli run-script $runner     --forward mpm_snow_ball --forward 20     --artifact-dir $outDir     --timeout 240 --json 2>&1 | Tee-Object -FilePath $logFile -Append
