# sensor_imu — B-route via newton-cli run-script.
# This example needs per-step Python (per-step IMU integration)
...
"=== sensor_imu via newton-cli run-script (B-route) ===" | Tee-Object -FilePath $logFile

& $py -m newton_cli run-script $runner     --forward sensor_imu --forward 300     --artifact-dir $outDir     --timeout 600 --json 2>&1 | Tee-Object -FilePath $logFile -Append
