# basic_conveyor — B-route via newton-cli run-script.
# This example needs per-step Python (a custom @wp.kernel drives the kinematic belt motion)
...
"=== basic_conveyor via newton-cli run-script (B-route) ===" | Tee-Object -FilePath $logFile

& $py -m newton_cli run-script $runner     --forward basic_conveyor --forward 30     --artifact-dir $outDir     --timeout 240 --json 2>&1 | Tee-Object -FilePath $logFile -Append
