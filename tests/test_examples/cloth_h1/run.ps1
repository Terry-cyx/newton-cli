# cloth_h1 — B-route via newton-cli run-script.
# This example needs per-step Python (a torch policy controls the H1 robot)
...
"=== cloth_h1 via newton-cli run-script (B-route) ===" | Tee-Object -FilePath $logFile

& $py -m newton_cli run-script $runner     --forward cloth_h1 --forward 20     --artifact-dir $outDir     --timeout 240 --json 2>&1 | Tee-Object -FilePath $logFile -Append
