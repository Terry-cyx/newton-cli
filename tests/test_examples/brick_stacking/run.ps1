# brick_stacking — B-route via newton-cli run-script.
# This example needs per-step Python (a multi-step task sequencer in Python)
...
# --test --num-frames 2000, which makes Newton call Example.test_final()
...
"=== brick_stacking via newton-cli run-script (B-route) ===" | Tee-Object -FilePath $logFile

& $py -m newton_cli run-script $runner     --forward brick_stacking --forward 2000     --artifact-dir $outDir     --timeout 1200 --json 2>&1 | Tee-Object -FilePath $logFile -Append
