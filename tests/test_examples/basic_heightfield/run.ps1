# basic_heightfield — 5 spheres dropped onto a wavy terrain via newton-cli.
...
"=== basic_heightfield via newton-cli ===" | Tee-Object -FilePath $logFile

"--- prep: generate 50x50 sin/cos elevation grid ---" |
...
"`n--- sim run (100 frames @ 100 fps, XPBD iter=10, cpu) ---" |
    Tee-Object -FilePath $logFile -Append
...
& $py $viz $stateOut $snapshot $summary --title "basic_heightfield (CLI)" 2>&1 |
    Tee-Object -FilePath $logFile -Append
