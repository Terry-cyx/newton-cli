# ik_franka — B-route via newton-cli run-script.
# This example needs per-step Python (IK example (empty test_final))
...
"=== ik_franka via newton-cli run-script (B-route) ===" | Tee-Object -FilePath $logFile

& $py -m newton_cli run-script $runner     --forward ik_franka --forward 30     --artifact-dir $outDir     --timeout 240 --json 2>&1 | Tee-Object -FilePath $logFile -Append
