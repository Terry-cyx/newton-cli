# ik_custom — B-route via newton-cli run-script.
# This example needs per-step Python (custom IK kernels)
...
"=== ik_custom via newton-cli run-script (B-route) ===" | Tee-Object -FilePath $logFile

& $py -m newton_cli run-script $runner     --forward ik_custom --forward 30     --artifact-dir $outDir     --timeout 240 --json 2>&1 | Tee-Object -FilePath $logFile -Append
