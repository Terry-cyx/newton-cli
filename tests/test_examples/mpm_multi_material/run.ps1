# mpm_multi_material — sand + snow + mud particle clusters dropping onto a static
# kinematic floor block. Exercises post_finalize.mpm_attrs which writes
# per-particle MPM material parameters via masked fill_(value).
...
"=== mpm_multi_material via newton-cli ===" | Tee-Object -FilePath $logFile
...
& $py $viz $stateOut $snapshot $summary --title "mpm_multi_material (CLI)" 2>&1 |
    Tee-Object -FilePath $logFile -Append
