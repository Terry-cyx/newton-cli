# sensor_tiled_camera — B-route via newton-cli run-script.
# This example needs per-step Python (tiled camera sensor with custom kernels)
...
"=== sensor_tiled_camera via newton-cli run-script (B-route) ===" | Tee-Object -FilePath $logFile

& $py -m newton_cli run-script $runner     --forward sensor_tiled_camera --forward 20     --artifact-dir $outDir     --timeout 240 --json 2>&1 | Tee-Object -FilePath $logFile -Append
