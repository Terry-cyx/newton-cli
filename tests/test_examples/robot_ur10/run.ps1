# robot_ur10 — B-route via newton-cli run-script.
# This example needs per-step Python (UR10 demo (empty test_final))
...
"=== robot_ur10 via newton-cli run-script (B-route) ===" | Tee-Object -FilePath $logFile

& $py -m newton_cli run-script $runner     --forward robot_ur10 --forward 30     --artifact-dir $outDir     --timeout 240 --json 2>&1 | Tee-Object -FilePath $logFile -Append
