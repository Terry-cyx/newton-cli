# cloth_twist — B-route via newton-cli run-script.
# This example needs per-step Python (custom kernels apply twist)
...
"=== cloth_twist via newton-cli run-script (B-route) ===" | Tee-Object -FilePath $logFile

& $py -m newton_cli run-script $runner     --forward cloth_twist --forward 20     --artifact-dir $outDir     --timeout 240 --json 2>&1 | Tee-Object -FilePath $logFile -Append
