# softbody_dropping_to_cloth — a soft tet block dropped onto a cloth sheet (VBD).
...
"=== softbody_dropping_to_cloth via newton-cli ===" | Tee-Object -FilePath $logFile
...
"`n--- sim run (60 frames @ 60 fps, 10 substeps, VBD iter=10, cuda:0) ---" |
    Tee-Object -FilePath $logFile -Append
& $py -m newton_cli sim run `
    ...
    --solver-arg particle_self_contact_radius=0.01 `
    --solver-arg particle_self_contact_margin=0.02 `
    --solver-arg particle_enable_tile_solve=true `
...
& $py $viz $stateOut $snapshot $summary --title "softbody_dropping_to_cloth (CLI)" 2>&1 |
    Tee-Object -FilePath $logFile -Append
