# robot_h1 — 2 Unitree H1 humanoids from downloaded USD asset (MuJoCo + cuda:0).
...
"=== robot_h1 via newton-cli ===" | Tee-Object -FilePath $logFile
...
& $py -m newton_cli sim run `
    ...
    --num-frames 50 --fps 50 --substeps 4 --device cuda:0 `
...
& $py $viz $stateOut $snapshot $summary --title "robot_h1 (CLI)" 2>&1 |
    Tee-Object -FilePath $logFile -Append
