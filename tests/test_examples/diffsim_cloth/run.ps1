# diffsim_cloth — B-route via newton-cli run-script.
...
"=== diffsim_cloth via newton-cli run-script (B-route) ===" | Tee-Object -FilePath $logFile

& $py -m newton_cli run-script $runner     --forward diffsim_cloth --forward 30     --artifact-dir $outDir     --timeout 240 --json 2>&1 | Tee-Object -FilePath $logFile -Append
