# cloth_hanging — 64x32 cloth grid hanging from fixed left edge under gravity.
...
"=== cloth_hanging via newton-cli ===" | Tee-Object -FilePath $logFile
...
& $py $viz $stateOut $snapshot $summary --title "cloth_hanging (CLI)" 2>&1 |
    Tee-Object -FilePath $logFile -Append
