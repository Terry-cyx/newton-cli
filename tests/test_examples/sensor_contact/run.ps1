# sensor_contact — B-route via newton-cli run-script.
# This example needs per-step Python (per-step contact sensor updates)
...
"=== sensor_contact via newton-cli run-script (B-route) ===" | Tee-Object -FilePath $logFile

& $py -m newton_cli run-script $runner     --forward sensor_contact --forward 20     --artifact-dir $outDir     --timeout 240 --json 2>&1 | Tee-Object -FilePath $logFile -Append
