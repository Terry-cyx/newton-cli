# robot_panda_hydro — B-route via newton-cli run-script.
# This example needs per-step Python (custom grip-control kernels)
...
# --test --num-frames 300, which makes Newton call Example.test_final()
...
"=== robot_panda_hydro via newton-cli run-script (B-route) ===" | Tee-Object -FilePath $logFile

& $py -m newton_cli run-script $runner     --forward robot_panda_hydro --forward 300     --artifact-dir $outDir     --timeout 600 --json 2>&1 | Tee-Object -FilePath $logFile -Append
