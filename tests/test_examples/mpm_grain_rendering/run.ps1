# mpm_grain_rendering — fully declarative.
# The example's test_final is just "all particles above the ground"; the
# render-grain machinery (sample_render_grains/update_render_grains) is
# cosmetic and doesn't affect physics validation, so the CLI version skips it.
...
"=== mpm_grain_rendering via newton-cli ===" | Tee-Object -FilePath $logFile
...
"`n--- sim run (30 frames @ 60 fps, 1 substep) ---" |
    Tee-Object -FilePath $logFile -Append
...
& $py $viz $stateOut $snapshot $summary --title "mpm_grain_rendering (CLI)" 2>&1 |
    Tee-Object -FilePath $logFile -Append
