...
which would corrupt our JSON stdout envelope. We capture that banner and
re-emit it on stderr only when the user asks for verbose output.
"""
...
def init_warp_silently() -> None:
    ...
    with _redirect_stdout_to_buffer() as buf:
        # Touching get_devices() forces wp.init() if it hasn't been called.
        wp.get_devices()
