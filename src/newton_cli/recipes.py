"""Recipe interpreter — declarative ModelBuilder driver.
...
re-execute its recipe — there is no opaque binary blob. This makes models
fully inspectable, version-control-friendly, and JSON-grep-able.
"""
...
RECIPE_SCHEMA = "newton-cli/recipe/v1"
...
def _to_transform(v: Any) -> wp.transform:
...
def _to_mesh_from_usd(spec: dict) -> Any:
    """Load a `newton.Mesh` from a USD stage + prim path.

    Spec:
        {"path": "/absolute/or/relative/path/to/file.usd",
         "prim": "/root/bunny"}
    Requires `newton[importers]` (pxr.Usd).
    """
...
def _to_mesh(spec: dict) -> Any:
    """Load a `newton.Mesh` from inline vertex+index arrays.

    Spec (minimum):
        {"vertices": [[x,y,z], ...], "indices": [i,j,k, ...]}

    Optional:
        is_solid: bool — passed to ``newton.Mesh(...)``
        compute_inertia: bool — passed to ``newton.Mesh(...)``
        build_sdf: dict — call ``mesh.build_sdf(**kwargs)`` after construction.
            Recognised kwargs: max_resolution, narrow_band_range (list[2]),
            margin, scale (list[3]).
    """
    import numpy as np  # noqa: PLC0415
...
def _coerce_value(v: Any) -> Any:
    """Heuristic shape-based coercion. Recursive for nested structures."""
    if isinstance(v, dict):
        # Tagged values: {"$shape_cfg": {...}}, etc. Tag must be the only key.
        if len(v) == 1:
            (only_key,) = v.keys()
    ...
    if _looks_like_quat_xyzw(v):
        return _to_quat(v)
    # Lists of vec3 / quat: builder methods like add_rod() require Warp types
    # for each element, not raw Python lists. Coerce uniformly when every
    # element matches the same shape (vec3-only or quat-only).
...
def _op_set_builder_array(builder: newton.ModelBuilder, args: dict) -> None:
    """Mutate one of the builder's mutable lists/arrays.

    Supports:
        {"name": "joint_q", "index": -1, "value": 1.5707}
        {"name": "joint_q", "slice": [start, stop], "values": [...]}
        {"name": "joint_target_ke", "fill": 150}
          -> broadcast the scalar to every element (length determined at runtime)
    """
    name = args["name"]
    ...
    if "slice" in args:
        ...
        if not isinstance(s, list) or len(s) not in (2, 3):
            raise RecipeError(f"set_builder_array: slice must be [start,stop] or [start,stop,step], got {s!r}")
        start = s[0]
        stop = s[1]
    ...
    if "fill" in args:
        value = args["fill"]
        # Optional 'range' field lets us fill a sub-range:
        #   {"name": "joint_target_ke", "fill": 500, "range": [6, null]}
        rng = args.get("range")
        if rng is None:
...
def _op_set_default_joint_cfg(builder: newton.ModelBuilder, args: dict) -> None:
    cfg = builder.default_joint_cfg
    for k, v in args.items():
        setattr(cfg, k, v)


def _op_set_default_shape_cfg(builder: newton.ModelBuilder, args: dict) -> None:
    cfg = builder.default_shape_cfg
    for k, v in args.items():
        setattr(cfg, k, v)


def _op_register_mujoco_custom_attributes(builder: newton.ModelBuilder, args: dict) -> None:
    """Register MuJoCo-specific custom attributes on the builder.

    Wraps `newton.solvers.SolverMuJoCo.register_custom_attributes(builder)`.
    Called once before add_usd/add_urdf/add_mjcf so that MuJoCo-native custom
    attributes get parsed out of the asset into the builder's internal tables.
    """
    newton.solvers.SolverMuJoCo.register_custom_attributes(builder)


def _op_register_solver_custom_attributes(builder: newton.ModelBuilder, args: dict) -> None:
    """Register a solver's custom attributes on the builder by solver name.

    Args:
        solver: class name in ``newton.solvers`` (e.g. ``SolverImplicitMPM``,
            ``SolverStyle3D``).
    """
    name = args["solver"]
...
def _op_pin_body(builder: newton.ModelBuilder, args: dict) -> None:
    """Pin a body in place by zeroing its mass and inertia.

    Args:
        body: body index to pin (int, negative values index from the end)
    """
...
def _op_apply_body_inertia_diagonal(builder: newton.ModelBuilder, args: dict) -> None:
    """Add a uniform diagonal offset to every body's 3x3 inertia tensor.

    Mirrors the example_basic_urdf.py stabilization trick:

        for body in range(body_count):
            inertia += eye(3) * value

    This is a standard "armature inertia" trick for stable contact-driven sims.
    """
    import numpy as np  # noqa: PLC0415
    import warp as wp  # noqa: PLC0415
...
def _op_replicate(builder: newton.ModelBuilder, args: dict) -> None:
    """Build a sub-builder from an inline recipe, then call outer.replicate(sub, count, spacing)."""
...
def _op_set_builder_attr(builder: newton.ModelBuilder, args: dict) -> None:
    """Set a scalar attribute on the builder itself.

    Args:
        name: attribute name (e.g. ``rigid_gap``)
        value: scalar value (number / bool / string)
    """
    name = args["name"]
...
def _execute_recipe_on_builder(builder: newton.ModelBuilder, recipe: dict) -> None:
    if recipe.get("schema") != RECIPE_SCHEMA:
        raise RecipeError(
            f"recipe schema must be {RECIPE_SCHEMA!r}, got {recipe.get('schema')!r}"
        )
    ops = recipe.get("ops")
    ...
    for i, entry in enumerate(ops):
        ...
        if not isinstance(raw_args, dict):
            raise RecipeError(f"op[{i}].args must be an object, got {type(raw_args).__name__}")

        # Special non-method ops are dispatched without coercion (they take
        # raw JSON values for indices, slice bounds, and array contents).
...
def _apply_mpm_attrs(model: newton.Model, entries: list) -> None:
    """Write per-particle MPM material attributes via masked fill.

    Each entry is one of:
        {"attr": "yield_pressure", "range": [lo, hi], "value": 2.0e4}
        {"attr": "yield_pressure", "indices": [0, 5, 9], "value": 2.0e4}

    Mirrors the example pattern:
        model.mpm.<attr>[wp_indices].fill_(value)

    `value` may be a scalar or a list of 3 numbers (-> wp.vec3).
    """
    import numpy as np  # noqa: PLC0415
    import warp as wp  # noqa: PLC0415
    ...
    for i, entry in enumerate(entries):
        ...
        if target is None:
            raise RecipeError(f"mpm_attrs[{i}]: model.mpm has no attribute {attr!r}")
        raw = entry["value"]
        ...
        if "range" in entry:
            ...
            # No selector — fill the whole array.
            target.fill_(value)


def _apply_post_finalize(model: newton.Model, recipe: dict) -> None:
    """Apply top-level `post_finalize` dict of model attributes after finalize().

    Generic form (plain attribute setter):
        "post_finalize": {"soft_contact_ke": 100.0, "soft_contact_kd": 0}

    Structured forms (recognized by reserved key, dispatched separately):
        "post_finalize": {"mpm_attrs": [
            {"attr": "yield_pressure", "range": [N0, N1], "value": 2.0e4},
            ...
        ]}

    Generic and structured forms can coexist in one block.
    """
    ...
    if "model_calls" in post:
        ...
        for i, call in enumerate(calls):
            ...
            if not callable(method):
                raise RecipeError(f"model_calls[{i}]: model has no method {method_name!r}")
            args = call.get("args", [])
            kwargs = call.get("kwargs", {})
            # Coerce list-args via the same shape rules as recipe ops.
            args = [_coerce_value(a) for a in args]
            kwargs = {k: _coerce_value(v) for k, v in kwargs.items()}
            method(*args, **kwargs)


def build_model_from_recipe(recipe_path: str | Path, *, device: str | None = None) -> newton.Model:
    """Re-execute a recipe and return the finalized Model."""
    path = Path(recipe_path)
    recipe = json.loads(path.read_text())
    with wp.ScopedDevice(device) if device else _nullctx():
...
import contextlib  # noqa: E402  (kept near use site)
